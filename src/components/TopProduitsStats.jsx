      ...
      try {
        ...
        if (!rows.length) {
          ...
          rows = topProducts.map((p) => ({
            ...
            pct: total > 0 ? Math.round((p.total_vendu / total) * 100) : 0,
        ...
        // Recalculer les pourcentages pour que tout s'additionne à 100%
        const totalQuantity = normalizedData.reduce((sum, p) => sum + p.quantity, 0);
        const correctedData = normalizedData.map((p) => ({
          ...p,
          pct: totalQuantity > 0 ? Math.round((p.quantity / totalQuantity) * 100) : 0,
